...
# 23/05/04
  * 오늘 오전은 막힘없이 진행된 느낌?
  * 오후에는 이미지를 가져오고 이미지가 없다면 if문을 작성하는데 시간을 많이 사용함


# 23/05/08

## 프로젝트 마무리
  * 23/05/05 부터 내용 작성이 없는 이유..
    * 늦개까지 작업하면서 내일쓰자 내일쓰자 미루면서 못씀.. 지금 쓰려니까 기억이 안남... 다음부터는 매일매일 쓰도록 노력해야겠다.
  * 마무리하며..
    * 좋은 팀원을 만나서 프로젝트하면서 스트레스없이 재미있게 프로젝트 할수 있었다 비록 완벽하고 원하던 기능을 구현하지 못했고
      다른 팀들보다 잘한것 같지는 않지만 프로젝트로 인한 팀원들과의 팀워크 많은 내용들의 복습 그리고 팀원들의 코드를 보고 피드백을 받고 주며 배운점은 많다고 생각한다.
  * 앞으로 프로젝트
    * 바로 프로젝트에 들어가는게 힘든걸 알면서 하니까 으악이라는 기분이지만 이번 프로젝트는 무엇을 더 배울지 또 얼마나 열정있는 팀원들과 팀을 이루어 더 좋은 프로젝트를 만들지 기대도 되고 있다. 내가 앞장서서 이끌고 하자! 이렇게 까지는 힘들지만 남들보다 뒤처지지 않게 열정있게 참여 할 자신은 있다. 이번 프로젝트의 목표는 조금더 완성도를 높이고 하지 못했던 기능들을 추가(이메일 인증, 카카오,네이버,구글 계정 로그인, 채팅 기능 등등 하고 싶은게 너무 많다)해 보며 다음주에도 성장하며 또 작성 하기를 기대한다.
