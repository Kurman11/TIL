...
# 23/04/29
  ## 회원가입/로그인 페이지 만들기
  * 회원가입  
    * JS쪽 작성을 chatgpt를 사용해 작성했는대 차후 공부후 내가 직접 구현해 보고싶다. 
    * 발견한 이슈
      * 회원가입 form.as_p가 아닌 form.xxx 으로 따로 받아 와서 그런지 모르겠지만 회원가입 데이터가 db에 저장이 안됨
      * 해결은 월요일 까지 페이지 다 만들고 추후에 찾아봐야 할듯 
  * 로그인
    * 별 이슈없이 편하게 만듦

  ### 만들면서 추가로 구현? 팀원분들께 건의 해보고 싶은 사항  
    1. 회원가입 > 이메일 인증하기 누르면 실제로 이메일이 날라오는 기능  
    2. 이메일 인증하기 버튼을 이메일이 작성이 안되었다면 diable하게 적용(이부분은 금방 가능 할 지도?)
    3. 체크박스 필수를 체크하지 않으면 회원가입이 불가능하게
    4. 비밀번호 재설정 html 만들기
  
### 정리
  * 개인적인 작은 이슈가 있어서 해결하고 평일동안 부족했던 잠을 낮잠으로 자고 하면서 진도가 느렸지만 html/css 스터디를 해온 것이 많은 도움이 된거 같다.
  아직 많이 부족하지만 css 관리가 처음보다 정리를 잘 하고 html 작성도 속도가 조금 늘었다.


# 23/05/01 ~ 02
  ## profile 페이지 만들기
  * 페이지는 어느정도 완성 했으나 정확하게 어떠한 데이터를 좋아요가 의미하는게 무엇인지 알수 없어서 완성을 못함
    * 오늘 회의를 통해 이야기 했지만 정확하게 정리를 못해 내일 데일리 스크럼을 통해 확실히 정리 해서 기록해야함
  ## 이슈
  * image를 한번에 여러장 넣기위해 시간을 많이 사용함
    * 해결 : 밑의 코드를 admin.py에 작성하고 admin 관리자 페이지에서 내용을 작성하여 데이터를 작성해줌
    ```python
    class ProductMultipleImageAdmin(admin.StackedInline):
    model = ProductImages
    ```
  * signup페이지에서 회원가입이 안되는 현상
    * 해결 : url 경로를 잘못 지정해 줘서 회원가입 form이 적용이 안되었음
